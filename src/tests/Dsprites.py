...
from my_data.dsprite.dspriteBilevel_original import *
...
inner_lr = 1e-5
inner_optimizer = torch.optim.Adam(inner_model.parameters(), lr=inner_lr, weight_decay=1e-3)
inner_scheduler = None

# Optimizer that improves the approximation of a*
inner_dual_lr = 1e-5
inner_dual_optimizer = torch.optim.Adam(inner_dual_model.parameters(), lr=inner_dual_lr, weight_decay=5e-3)
inner_dual_scheduler = None

# The outer neural network parametrized by the outer variable
outer_param = torch.cat((torch.rand(u_dim).to(device), state_dict_to_tensor(outer_model, device)), 0)
outer_lr = 1e-5
# Optimizer that improves the outer variable
outer_optimizer = torch.optim.Adam([outer_param], lr=outer_lr, weight_decay=1e-3)
outer_scheduler = None#StepLR(outer_optimizer, step_size=10000, gamma=0.1)

# Loss helper functions
MSE = nn.MSELoss()

# Print configuration
run_name = "Dsprites bilevel::="+" inner_lr:"+str(inner_lr)+", dual_lr:"+str(inner_dual_lr)+", outer_lr:"+str(outer_lr)+", max_inner_iters:"+str(max_inner_dual_iters)+", max_daul_iters:"+str(max_inner_iters)+", lam1:"+str(lam1)+", lam2:"+str(lam2)+", batch_size:"+str(batch_size)+", max_epochs:"+str(max_epochs)
...
def fo(outer_param, Vg_z_out, Y):
    feature = augment_stage2_feature(Vg_z_out)
    ...
    wandb.log({"out. loss term1": (MSE(pred, Y)).item()})
...
def fi(outer_param, g_z_in, X):
    ...
    #V = outer_param[-u_dim:]
    pred = g_z_in
    loss = MSE(pred, treatment_feature)
