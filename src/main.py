...
# Gradients
og1 = lambda mu, theta, X_out, y_out: (torch.full((n,1), 0.)).to(device)
og2 = lambda mu, theta, X_out, y_out: X_out.T @ (X_out @ theta - torch.reshape(y_out[:,0], (y_out[:,0].size()[0],1)))
ig22 = lambda mu, theta, X_in, y_in: X_in.T @ X_in + mu[0]*X_in.T @ X_in + mu[1]*X_in.T @ X_in
ig12 = lambda mu, theta, X_in, y_in: torch.cat((X_in.T @ (X_in @ theta - torch.reshape(y_in[:,1], (y_in[:,1].size()[0],1))), X_in.T @ (X_in @ theta - torch.reshape(y_in[:,2], (y_in[:,2].size()[0],1)))),1)
grads = [og1,og2,ig22,ig12]

# Optimize using classical implicit differention
bp_classic = BilevelProblem(outer_objective=fo, inner_objective=fi, method="implicit_diff", data=dataset, find_theta_star=find_theta_star, gradients=grads)
...
############################################################
####### CLASSICAL IMPLICIT DIFFERENTIATION AUTOGRAD ########
############################################################

# Optimize using classical implicit differention
bp_classic_aut = BilevelProblem(outer_objective=fo, inner_objective=fi, method="implicit_diff", data=dataset, find_theta_star=find_theta_star)
mu_opt_c_aut, iters_c_aut, n_iters_aut, times_aut, inner_loss_aut, outer_loss_aut, theta_aut = bp_classic_aut.optimize(mu0, maxiter=maxiter, step=step)
plot_loss(figures_dir+"inn_loss_CID_aut", inner_loss_aut, title="Inner loss of clas. im. diff. + aut.")
plot_loss(figures_dir+"out_loss_CID_aut", outer_loss_aut, title="Outer loss of clas. im. diff. + aut.")

# Show results
print("CLASSICAL IMPLICIT DIFFERENTIATION AUTOGRAD")
print("Argmin of the outer objective:", mu_opt_c_aut)
print("Average iteration time:", np.average(times_aut))
print("Number of iterations:", n_iters_aut)
print("True coefficients theta:", coef)
print("Fitted coefficients theta:", theta_aut)
print()
...
h_true = lambda X : X.to(device) @ coef
h_theta = lambda theta : lambda X : X.to(device) @ theta
h_star_c = bp_classic.h_star(h_theta)
h_theta_aut = lambda theta : lambda X : X.to(device) @ theta_aut
h_star_aut = bp_classic_aut.h_star(h_theta_aut)

f_c = lambda mu : torch.pow(torch.norm((h_star_c(X_train) - y_train)),2) + (mu.clone().detach())*torch.pow(torch.norm(h_star_c(X_train)),2)
f_c_aut = lambda mu : torch.pow(torch.norm((h_star_aut(X_train) - y_train)),2) + (mu.clone().detach())*torch.pow(torch.norm(h_star_aut(X_train)),2)
plot_2D_functions(figures_dir+"pred_function", h_true, h_star_c, h_star_aut, device, points=None, plot_x_lim=[0,mu_0_value], plot_y_lim=[0,1], plot_nb_contours=80, titles=["True Imp. Diff.","Classical Imp. Diff.","Class. Imp. Diff. + aut."])
...
fi_h_X = lambda mu, h_X_in, y_in: ((1/2)*torch.mean(torch.pow((h_X_in - y_in[:,0]),2))) + mu[0]*((1/2)*torch.mean(torch.pow((h_X_in - y_in[:,1]),2))) + mu[1]*((1/2)*torch.mean(torch.pow((h_X_in - y_in[:,2]),2)))
grads = [og1,og2,ig22,ig12]
...
bp_neural = BilevelProblem(outer_objective=fo, inner_objective=fi, method="neural_implicit_diff", data=dataset, fo_h_X=fo_h_X, fi_h_X=fi_h_X, device=device, gradients=grads)
...
plot_2D_functions(figures_dir+"pred_function", h_true, h_star_c, h_star_n, device, points=None, plot_x_lim=[0,mu_0_value], plot_y_lim=[0,1], plot_nb_contours=80, titles=["True Imp. Diff.","Classical Imp. Diff.","Neural Imp. Diff."])
"""
