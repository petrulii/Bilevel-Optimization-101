...
device = "cuda"
#"closed_form_DFIV", "closed_form_a", "GD", "GDinH", "torch_autograd"
grad_method = "closed_form_DFIV"
...
class InnerSolution(nn.Module):
  ...
  def optimize_dual_inH(self, outer_param, Z_outer, X_outer, outer_grad):
    """
  ...
  def optimize_dual(self, outer_param, Z_outer, X_outer, outer_grad):
    """
    Optimization loop for the inner-level model that approximates a*.
    """
    total_loss, total_epochs, total_iters = 0, 0, 0
...
class ArgMinOp(torch.autograd.Function):
  ...
  def backward(ctx, outer_grad):
    ...
    match grad_method:
      case "GD":
        # We do not want to train h* in the backward pass, only a*.
        inner_solution.evaluate = True
        ...
        with torch.enable_grad():
          ...
          inner_solution.optimize_dual(outer_param, Z_outer, X_outer, outer_grad)
          inner_solution.dual_model.eval()
        with torch.no_grad():
          h_X_o = inner_solution.model(Z_outer)
          dual_value = inner_solution.dual_model(h_X_o)
          grad = inner_solution.compute_hessian_vector_prod(outer_param, Z_outer, X_outer, inner_value, dual_value)
      case "GDinH":
        # We do not want to train h* in the backward pass, only a*.
        inner_solution.evaluate = True
        # Need to enable_grad because we use autograd in optimize_dual (disabled in backward() by default).
        with torch.enable_grad():
          ...
          # in the outer loop where we optimize the outer objective g(outer_param, h).
          inner_solution.dual_model.train(True)
          inner_solution.optimize_dual_inH(outer_param, Z_outer, X_outer, outer_grad)
          inner_solution.dual_model.eval()
        with torch.no_grad():
          dual_value = inner_solution.dual_model(Z_outer)
          grad = inner_solution.compute_hessian_vector_prod(outer_param, Z_outer, X_outer, inner_value, dual_value)
      case "closed_form_a":
        # We do not want to train h* in the backward pass, only find optimal a*.
        inner_solution.evaluate = True
        # Get outer Y
        ...
        f_X_i = torch.func.functional_call(inner_solution.outer, parameter_and_buffer_dicts=outer_NN_dic, args=X, strict=True)
        res = fit_2sls_noprint(f_X_i, h_X_i, h_X_o, Y, inner_solution.lam_V, inner_solution.lam_u)
        u = res["stage2_weight"]
